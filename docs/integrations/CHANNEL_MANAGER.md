...
## 2026-05-08 Production-Handoff Update

The current source code intentionally treats direct OTA APIs as staged until vendor approval/certification is complete. Booking.com, Agoda, and Airbnb worker endpoints are protected and rate-limited, but they return `skipped` rather than fake success when required vendor credentials or certified adapters are not available.

Use this order for live rollout:

1. Launch core PMS/direct booking first.
2. Use an aggregator such as HotelRunner/MyAllocator for immediate OTA coverage if needed.
3. Enable direct Booking.com/Agoda/Airbnb workers only after provider approval, sandbox certification, and live credential verification.
4. Attach webhook delivery logs and worker logs to `MASTER_4P_TASKS.md` evidence before enabling live inventory sync.
