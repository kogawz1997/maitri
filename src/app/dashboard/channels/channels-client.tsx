...
export function ChannelsClient({ hotelId, hotelName, connections, roomTypes }: {
  ...
  const [configForm, setConfigForm] = useState({ apiKey: '', webhookToken: '', propertyId: '', active: true });
  ...
  async function saveConfig() {
    ...
    const payload = {
      ...
      status: configForm.active ? 'active' : 'disabled',
      external_property_id: configForm.propertyId || null,
      credentials: {
        api_key: configForm.apiKey || null,
        webhook_token: configForm.webhookToken || null,
      },
      config: { mode: 'direct', active: configForm.active },
    };
    let error;
    if (existing) {
  ...
  async function syncChannel(channelId: string) {
    ...
    setSyncing(null);
    toast.info('สร้างหน้างาน sync แล้ว — การ sync จริงต้องใช้ vendor approval/credentials และ endpoint worker');
  }

  async function disconnect(channelId: string) {
    const conn = connMap.get(channelId);
    if (!conn) return;
    await supabase.from('channel_connections').update({ status: 'disabled' }).eq('id', conn.id);
    const { data } = await supabase.from('channel_connections').select('*').eq('hotel_id', hotelId);
    setConns(data || []);
                  ...
                  {isConnected ? (
                    <>
                      <Button size="sm" variant="outline" className="flex-1"
                        onClick={() => { setConfigChannel(ch); setConfigForm({ apiKey: conn.credentials?.api_key || '', webhookToken: conn.credentials?.webhook_token || '', propertyId: conn.external_property_id || '', active: true }); }}>
                    ...
                    <>
                      <Button size="sm" className="flex-1" disabled={ch.comingSoon}
                        onClick={() => { setConfigChannel(ch); setConfigForm({ apiKey: '', webhookToken: '', propertyId: '', active: true }); }}>
      ...
      <Dialog open={!!configChannel} onOpenChange={o => !o && setConfigChannel(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Field label="External Property ID / Hotel ID" value={configForm.propertyId}
              onChange={v => setConfigForm(p => ({ ...p, propertyId: v }))}
              placeholder="Property ID จาก OTA/Extranet" />
