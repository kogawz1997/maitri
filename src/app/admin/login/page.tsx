...
import { loginWebAdmin } from '@/lib/auth/role-login';
...
export default function RoleLoginPage() {
  ...
  async function handleRoleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const result = await loginWebAdmin(email, password);
      if (!result.ok) {
        toast.error(result.message);
        return;
      }
      toast.success('เข้าสู่ระบบผู้ดูแลเว็บสำเร็จ');
  ...
  return (
    <form onSubmit={handleRoleLogin} className="space-y-4">
      ...
      <Button type="submit" className="w-full" disabled={loading}>
        {loading ? 'กำลังเข้าสู่ระบบ...' : <>เข้าสู่ระบบเว็บแอดมิน<ArrowRight /></>}
      </Button>
    </form>
  );
}
